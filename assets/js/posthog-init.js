(function () {
  ...
  fetch("/api/posthog-config")
    ...
    .then(function (config) {
      ...
      // PostHog's official bootstrap snippet (de-minified, otherwise
      // unmodified). array.js's own bootstrap code looks for a
      // window.posthog._i array of [token, config, name] tuples -- a
      // generic [method, ...args] push queue (what this file used to build
      // by hand) is a different shape and array.js never reads it, so
      // init() silently never runs.
      !function (t, e) {
        var o, n, p, r;
        ...
        loaded: function (posthog) {
          fetch("/api/posthog-identity")
            ...
            .then(function (identity) {
              ...
              var properties = identity.email ? { email: identity.email } : {};
